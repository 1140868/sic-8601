...
@Component({
  selector: 'app-produto-registo',
  templateUrl: './produto-registo.component.html',
  styleUrls: ['./produto-registo.component.css']
})
export class ProdutoRegistoComponent implements OnInit {
  ...
  ngOnInit() {
    this.selectedWidthMax = 0;
    this.selectedHeightMax = 0;
    this.selectedDepthMax = 0;

    this.productService.getDotnetMaterials().subscribe((data) => { this.availableMaterials = data; });
    this.productService.getProdutos().subscribe((data) => { this.components = data; });
          ...
          this.productService.createDotnetProduct({
            ...
            (error) => {
              ...
              if (error.error = "SyntaxError") {
                alert("Product was created.")
                window.location.reload();
              } else { alert("Could not create product: " + error); }
            },
            () => { }
          );
        },
